<?php
class customerEntity extends Database{
    protected function getOpenOrdersID(){
        $currentUser = $_SESSION['userId'];
        // Retrieve open orders from the database.
        $sql =  "SELECT `order`.`id`, `userdirectory`.`Name`,`order`.`table_code`, `order`.`comment`, `order`.`grand_total`, `order`.`total`, `order`.`created_at`, `order`.`closed_at`
                 FROM `order` INNER JOIN `userdirectory` ON `order`.`userId`=`userdirectory`.`UserId` 
                 WHERE STATUS = 0 AND `order`.`userId` = $currentUser ORDER BY `id` ASC;";
