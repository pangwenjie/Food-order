...
<?php
	class profileController extends profileEntity{
		function retrieveProfile($id){
			$result = $this->getUserWithId($id);
				...
				&& (isset($_POST['emailError']) !== '')){
					if((isset($_POST['nameError']) == true) 
						...
						&& (isset($_POST['emailError']) == true)){
							...
							$result1 = $this->checkForChanges($name, $password, $dateOfBirth, $phoneNumber, $email, $id);

							$result2 = $this->checkForDuplicates($name, $dateOfBirth, $phoneNumber, $email, $id);
							
							if(($result1->num_rows == 0) && ($result2->num_rows == 0)){

								$result = $this->updateUser($name, $password, $dateOfBirth, $phoneNumber, $email, $id);
								
								if ($result === TRUE) {
									echo '<script>alert("User updated successfully"); window.location.href="viewProfile.php";</script>';
								} else {
